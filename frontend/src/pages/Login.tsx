...
export default function Login() {
  ...
  // Google SSO Handler: Auto-fills Email & Password. Auto-fills Company Name ONLY for Business Emails (@empresa.com)
  const handleGoogleLogin = async () => {
    ...
    setTimeout(() => {
      // Detect if user typed a custom email or fallback to test business email
      const targetEmail = email && email.includes('@') ? email : 'contato@studiopro.com.br';
      const domain = targetEmail.split('@')[1]?.toLowerCase() || 'gmail.com';
      const personalDomains = ['gmail.com', 'hotmail.com', 'outlook.com', 'yahoo.com', 'yahoo.com.br', 'icloud.com', 'live.com', 'bol.com.br', 'uol.com.br'];
      const isBusiness = !personalDomains.includes(domain);

      setEmail(targetEmail);
      setPassword('GoogleAuth#2026!');
      setConfirmPassword('GoogleAuth#2026!');
      setMode('register');
      setIsLoading(false);

      if (isBusiness) {
        // E-mail Business -> Autopreenche o Nome da Empresa
        const companyFromDomain = domain.split('.')[0];
        const formattedName = companyFromDomain.charAt(0).toUpperCase() + companyFromDomain.slice(1) + ' Studio';
        setNomeEmpresa(formattedName);
        setGoogleSuccessMessage(`✓ E-mail Business (@${domain}) detectado! Nome da empresa ("${formattedName}") autopreenchido. Preencha o WhatsApp e Nicho para prosseguir.`);
      } else {
        // E-mail Normal / Pessoal (@gmail.com) -> NÃO autopreenche o nome da empresa
        setNomeEmpresa('');
        setGoogleSuccessMessage(`✓ E-mail Pessoal (@${domain}) vinculado! E-mail e senhas autopreenchidos. Digite o Nome da Empresa, WhatsApp e Nicho.`);
      }
    }, 600);
  };
        ...
        <div className="relative rounded-[28px] p-[1px] bg-gradient-to-b from-white/35 via-white/10 to-white/[0.03] shadow-[0_25px_80px_rgba(0,0,0,0.95)]">
          <div className="rounded-[27px] bg-[#121215]/90 backdrop-blur-3xl p-7 sm:p-8 flex flex-col">
            ...
            {googleSuccessMessage && (
              <div className="mb-4 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl text-xs text-emerald-400 text-left flex items-start gap-2">
